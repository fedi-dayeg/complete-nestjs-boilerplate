...
export class NotificationOutboxService implements OnModuleInit {
    ...
    async enqueueFanout(payload: INotificationOutboxPayload): Promise<void> {
    ...
    async dispatchPending(): Promise<void> {
        ...
        await Promise.all(
            pending.map(event => this.enqueueHandleJob({ outboxId: event.id }))
        );
    }
    ...
    async handleOutbox(outboxId: string): Promise<void> {
        const now = this.helperService.dateCreate();
        const canProcess =
            await this.notificationOutboxRepository.markProcessing(
                outboxId,
                now
            );
        if (!canProcess) {
        ...
        const event =
            await this.notificationOutboxRepository.findById(outboxId);
        if (!event) {
            return;
        }
    ...
    private async handleNotificationFanout(event: OutboxEvent): Promise<void> {
    ...
    private async handleOutboxFailure(
        ...
        const delay = NotificationOutboxRetryDelayMs * Math.max(attempts, 1);
