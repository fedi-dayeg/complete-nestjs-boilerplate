...
import { E2E_USER_ACCESS_TOKEN_PAYLOAD_TEST, E2E_USER_PERMISSION_TOKEN_PAYLOAD_TEST } from "../user/user.constant";
import {
  E2E_API_KEY_ADMIN_CREATE_URL,
  E2E_API_KEY_ADMIN_GET_URL,
  E2E_API_KEY_ADMIN_LIST_URL, E2E_API_KEY_ADMIN_UPDATE_NAME_URL, E2E_API_KEY_ADMIN_UPDATE_RESET_URL
...
describe('E2E Api Key', () => {
  ...
  it(`GET ${E2E_API_KEY_ADMIN_GET_URL} GET Success`, async () => {
    const response = await request(app.getHttpServer())
      .get(
        E2E_API_KEY_ADMIN_GET_URL.replace(':_id', apiKey._id))
      .set('Authorization', `Bearer ${accessToken}`)
      .set('x-permission-token', permissionToken);
    expect(response.status).toEqual(HttpStatus.OK);
    expect(response.body.statusCode).toEqual(HttpStatus.OK);
  ...
  it(`PATCH ${E2E_API_KEY_ADMIN_UPDATE_RESET_URL} Expired`, async () => {
    const response = await request(app.getHttpServer())
      .patch(E2E_API_KEY_ADMIN_UPDATE_RESET_URL.replace(
        ':_id',
        apiKeyExpired._id
      ))
      .set('Authorization', `Bearer ${accessToken}`)
  ...
  it(`PUT ${E2E_API_KEY_ADMIN_UPDATE_NAME_URL} Success`, async () => {
    const response = await request(app.getHttpServer())
      .put(E2E_API_KEY_ADMIN_UPDATE_NAME_URL.replace(
        ':_id',
        apiKey._id
      ))
      .send({
